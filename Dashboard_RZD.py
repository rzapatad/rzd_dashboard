...
def load_institutional_data():
    ...
    try:
        ...
        cols_map = {'Item': 'Item', 'Familia': 'Familia', 'Descripción': 'Descripcion', 'Costo': 'Costo', 'Empresa': 'Empresa', 'Fase de Tesis': 'Fase', 'Producto': 'Producto'}
        ...
        if os.path.exists(db_file):
            ...
            df_final = df_final.drop(columns=['Estado']).merge(df_saved[['Descripcion', 'Estado']], on='Descripcion', how='left')
...
st.sidebar.title("🔐 Acceso Editor")
try:
    master_key = st.secrets["ADMIN_TOKEN"]
except:
    master_key = "rz2026" 
password_input = st.sidebar.text_input("Contraseña:", type="password")
es_admin = (password_input == master_key)
...
if not st.session_state.df_pro.empty:
    ...
    tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs([
        "📝 Gestión Administrativa", "📊 Análisis Financiero", "📊 Riesgos (Pareto)", 
    ...
    with tab1:
        ...
        if not es_admin: st.warning("Modo Lectura: Ingrese clave en la barra lateral para editar.")
        ...
        edited_df = st.data_editor(df_visible, column_config={"Estado": st.column_config.SelectboxColumn("Estado", options=opciones, required=True), "Costo": st.column_config.NumberColumn("Presupuesto", format="S/ %.2f")}, disabled=df_visible.columns if not es_admin else ["Item", "Familia", "Descripcion", "Costo"], hide_index=True, use_container_width=True)
        if es_admin:
    ...
    with tab2:
        color_map = {"Comprado y entregado": "#064e3b", "Comprado y en proceso de entrega": "#10b981", "En proceso de compra": "#f59e0b", "En proceso de Aprobación para Compra": "#3b82f6", "En proceso de solicitud de compra": "#6366f1"}
        c1, c2 = st.columns(2)
        with c1: st.plotly_chart(px.pie(st.session_state.df_pro, values='Costo', names='Estado', hole=0.5, color='Estado', color_discrete_map=color_map, title="Distribución por Estado"), use_container_width=True)
        with c2: st.plotly_chart(px.bar(st.session_state.df_pro, x='Familia', y='Costo', color='Estado', color_discrete_map=color_map, barmode='stack', text_auto='.2s', title="Ejecución por Familia"), use_container_width=True)
    ...
    with tab3:
        ...
        with c1:
            ...
            st.plotly_chart(px.bar(df_p, x='Costo', y='Descripcion', orientation='h', color='Costo', color_continuous_scale='Reds', title="Top 10 Mayores Inversiones"), use_container_width=True)
        with c2:
            if 'Empresa' in st.session_state.df_pro.columns:
                df_emp = st.session_state.df_pro.groupby('Empresa')['Costo'].sum().reset_index()
                st.plotly_chart(px.funnel(df_emp.sort_values(by='Costo', ascending=False).head(10), x='Costo', y='Empresa', title="Concentración por Proveedor"), use_container_width=True)

    with tab4:
        st.subheader("🎯 Proyecciones de Cierre Presupuestal")
        st.plotly_chart(px.scatter(st.session_state.df_pro, x="Costo", y="Estado", size="Costo", color="Fase", hover_name="Descripcion", size_max=60, color_discrete_map={"Fase II": "#10b981", "Fase III": "#f59e0b"}, title="Carga Financiera por Fase y Estado"), use_container_width=True)
    ...
    with tab5:
        ...
        with col_aud1:
            ...
            st.write("**Semáforo de Ejecución por Familia**")
        ...
        with col_aud2:
            ...
            fig_radar = go.Figure(go.Scatterpolar(r=radar_close['Costo'], theta=radar_close['Producto'], fill='toself', line_color='#1e3a8a'))
            fig_radar.update_layout(polar=dict(radialaxis=dict(visible=True, showticklabels=True, tickfont=dict(size=10)), angularaxis=dict(tickfont=dict(size=12))), showlegend=False, height=450, margin=dict(t=40, b=40, l=60, r=60))
    ...
    with tab6:
        ...
        with c1: st.plotly_chart(px.bar(df_fase, x='Fase', y='Costo', color='Situación', barmode='group', color_discrete_map={'Ejecutado': '#064e3b', 'Pendiente': '#6366f1'}, text_auto='.2s', title="Estatus por Fase Experimental"), use_container_width=True)
        with c2: 
        ...
        st.divider()
        st.subheader("🚨 Detalle de Insumos Pendientes Segmentados")
    ...
    with tab7:
        st.subheader("📦 Clasificación Presupuestal por Producto")
        c1, c2 = st.columns(2)
        with c1: st.plotly_chart(px.pie(st.session_state.df_pro, values='Costo', names='Producto', hole=0.4, color_discrete_sequence=px.colors.qualitative.Prism, title="Gasto por Categoría"), use_container_width=True)
        with c2: 
            df_prod_sum = st.session_state.df_pro.groupby('Producto')['Costo'].sum().reset_index().sort_values(by='Costo', ascending=True)
            st.plotly_chart(px.bar(df_prod_sum, x='Costo', y='Producto', orientation='h', color='Costo', color_continuous_scale='Viridis', text_auto='.2s', title="Montos Totales por Producto"), use_container_width=True)
        st.plotly_chart(px.treemap(st.session_state.df_pro, path=['Familia', 'Producto', 'Descripcion'], values='Costo', color='Producto', color_discrete_sequence=px.colors.qualitative.Bold, title="Mapa Jerárquico de Inversión"), use_container_width=True)
